...
if __name__ == '__main__':
    ...
    group = parser.add_mutually_exclusive_group(required=True)#创建互斥参数
    group.add_argument(
        '--exp',
        type=int,
        default=None,
    ...
    parser.add_argument(
        '--machine', type=str, choices=['local'], default='local') #加入参数--machine,如果参数前没有-or --, 则为必填参数
    parser.add_argument('--debug', action='store_true')#action：当该参数值为"store_true"时表示创建的是一个机关，带该参数时则表示开启该机关(创建的参数(机关)此时的值为True)，不带该参数时则表示关闭该机关(创建的参数(机关)此时的值为False)。
    args = parser.parse_args() #获取程序参数的值，args是一个对象，该对象里的若干属性等于参数(提取参数的值：args.参数名称)
